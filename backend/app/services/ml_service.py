...
# Merged from backend-dinesh — handles downloading from Supabase, extracting, running pipeline
import zipfile
import tempfile
...
class MLService:
    ...
    # -------------------------------------------------------
    # 3) FIND pipeline.py (unified path resolution)
    # -------------------------------------------------------
    @staticmethod
            ...
            try:
                ...
                if any(indicator in msg for indicator in duplicate_indicators):
                    try:
                        ...
                        raise Exception(f"Upload failed after attempting remove. original: {msg}, retry error: {e2}")
                else:
